...
################
# Get the directory with the exec outputs
...
"$cxx_compiler" ../source/cppfront.cpp --std=c++20 -o "$cppfront_cmd"
if [[ $? -ne 0 ]]; then
...
for test_file in $tests; do
    ...
    if [ -f "$expeced_output" ]; then
        diff_output=$(git diff -- "$expeced_output")
    ...
    if [ -f "$expected_src" ]; then
        diff_output=$(git diff -- "$expected_src")
        if [[ -n "$diff_output" ]]; then
            ...
            if [ -f "$expected_src_compil_out" ]; then
                ...
                if [[ $? -ne 0 ]]; then
                    ...
                    diff_output=$(git diff -- "$expected_src_compil_out")
                    if [[ -n "$diff_output" ]]; then
                        ...
                        (cd "$exec_out_dir"; ./test.exe > "$test_name.cpp.execution" 2>&1)
                        diff_output=$(git diff -- "$expected_src_exec_out")
                        if [[ -n "$diff_output" ]]; then
                        ...
                        if [[ -f "$expected_files" ]]; then
                            ...
                            for file in ${files/,/ }; do
                                diff_output=$(git diff -- "$exec_out_dir/$file")
                                if [[ -n "$diff_output" ]]; then
